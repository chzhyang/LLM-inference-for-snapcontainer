# /home/sdp/miniconda3/envs/llmenv/lib/python3.9/site-packages/transformers/generation/utils.py
# coding=utf-8
...
class GenerationMixin:
    ...
    def _get_logits_processor(
        ...
        if generation_config.guidance_scale is not None and generation_config.guidance_scale > 1:
            processors.append(ClassifierFreeGuidanceLogitsProcessor(
    ...
    def generate(
        ...
        perf: Optional[dict] = None,  # chzhyang
        **kwargs,
        ...
        if self.config.is_encoder_decoder and "encoder_outputs" not in model_kwargs:
            ...
            st_encoder = time.perf_counter()
            model_kwargs = self._prepare_encoder_decoder_kwargs_for_generation(
                inputs_tensor, model_kwargs, model_input_name
            )
            perf["encoder_time"] = time.perf_counter() - st_encoder
        ...
        # # chzhyang
        # print(f"is_constraint_gen_mode: {is_constraint_gen_mode}")
        # print(f"is_contrastive_search_gen_mode: {is_contrastive_search_gen_mode}")
        # print(f"is_greedy_gen_mode: {is_greedy_gen_mode}") # llama2 chatglm2 use greedy search
        # print(f"is_sample_gen_mode: {is_sample_gen_mode}")
        # print(f"is_beam_gen_mode: {is_beam_gen_mode}")
        # print(f"is_beam_sample_gen_mode: {is_beam_sample_gen_mode}")
        # print(f"is_group_beam_gen_mode: {is_group_beam_gen_mode}")
        # print(f"is_assisted_gen_mode: {is_assisted_gen_mode}")
    ...
    def greedy_search(
        ...
        **model_kwargs,
        ...
        this_peer_finished = False  # used by synced_gpus only
        latency = perf["latency"] if perf else []  # chzhyang
        while True:
            st = time.perf_counter()  # chzhyang
            try:  # chzhyang
                if synced_gpus:
                    # Under synced_gpus the `forward` call must continue until all gpus complete their sequence.
                    # The following logic allows an early break if all peers finished generating their sequence
                    this_peer_finished_flag = torch.tensor(
                        0.0 if this_peer_finished else 1.0).to(input_ids.device)
                    # send 0.0 if we finished, 1.0 otherwise
                    dist.all_reduce(this_peer_finished_flag,
                ...
                end = time.perf_counter()  # chzhyang
                latency.append(end - st)  # chzhyang
                # tmp = input_ids[0].tolist() # chzhyang
                # print(len(latency), len(tmp), tmp) # chzhyang
